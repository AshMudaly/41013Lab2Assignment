classdef Controller < handle
    ...
    methods
        ...
        function EStop(self)
            ...
            while self.stop == 1
                ...
                % Check if stop is disengaged
                if self.stop == 0
                    fprintf('Resuming operations\n');
                    configureCallback(self.serialObj, "terminator", @(src, event) self.ReadData(src, event));
                    break;
                end
            end
        ...
        % New function to set stop state from GUI
        function setGuiEStop(self, state)
            self.stop = state; % Update stop state based on GUI toggle
            if self.stop == 1
                fprintf('GUI E-Stop engaged\n');
            else
                fprintf('GUI E-Stop disengaged\n');
            end
        end
    ...
    methods(Static)
        ...
        %% all players stand
        function AllHit(self)
            currentQ = self.GetJointState(self.player);
            newQ = self.player.model.ikcon(transl(-0.05,-0.35,0.73), currentQ);
            self.player.model.fkine(newQ);
            % Create Matrix of movement between start position and end
            % position with 25 iterations
            moveQ = jtraj(currentQ,newQ,25);
            ...
            currentQ = self.GetJointState(self.player);
            newQ = self.player.model.ikcon(transl(-0.05,-0.35,0.72), currentQ);
            self.player.model.fkine(newQ);
            % Create Matrix of movement between start position and end
            % position with 25 iterations
            moveQ = jtraj(currentQ,newQ,3);
